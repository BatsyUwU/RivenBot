...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if(!args[0]) return Errors.wrongText(message, "You must provide a command to reload.");
        
        const commandName = args[0].toLowerCase();
        if(!bot.commands.get(commandName)) return Errors.wrongText(message, "That command doesn't exist. Try again.");
