    ...
    run: async (client, message, args) => {
        ...
        const steamEmbed = new MessageEmbed()
            ...
            .addField("__**Details**__", stripIndents`
                _Release Date:_ **${details.release_date ? details.release_date.date : "Coming Soon"}**
                _Price:_ **${price}**
                _Genres:_ **${details.genres.map((g) => g.description).join(", ")}**
                _Platform:_ **${platforms.join(", ") || "None"}**
                _Achievements:_ **${details.achievements ? formatNumber(details.achievements.total) : 0}**
                _DLC Count:_ **${details.dlc ? formatNumber(details.dlc.length) : 0}**
                _Recommendations:_ **${details.recommendations ? formatNumber(details.recommendations.total) : "None"}**
                ...
                _Support:_ **${details.support_info ? details.support_info.url : (details.support_info.email || "None")}**`)
