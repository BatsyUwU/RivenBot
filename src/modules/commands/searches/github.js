...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (args[0] && args[1]) {
            ...
            axios.get(`https://api.github.com/repos/${args[0]}/${args[1]}`).then((res) => {
                const repos = res.data;

                if(repos.message) {
                    return Errors.resStatus("404", message, `I wasnt able to find \`${args[0]}/${args[1]}\` on the github website!`);
                }
            ...
            axios.get(`https://api.github.com/users/${args[0]}`).then((res) => {
                ...
                if(users.message) {
                    return Errors.resStatus("404", message, `I wasnt able to find \`${args[0]}\` on the github website!`);
                }
